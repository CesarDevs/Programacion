...
def BusquedaPrimeroMejor(NodoStart, NodoEnd):
    ...
    nodosOrdenados = ordenamientoBurbuja(nodoActual, NodoEnd, vecinos)
    print(nodosOrdenados)
    #Agregar los nodos ordenados a la cola
    #Hacer una funcion para agregar a la cola en orden sin los ya visitados
    AgregarListaACola(nodosOrdenados)
        
    #Se hacer la recurcibidad
    print(cola)
    
    while(cola[0] in visitados):
        cola.pop(0)
    
    BusquedaPrimeroMejor(cola[0], NodoEnd)
...
def AgregarListaACola(Lista):
    ...
    for index in enumerate(Lista):
        ...
        if (Lista[index[0]] not in visitados):
            temp.append(Lista[index[0]])

    cola = temp+cola
    print("se agrego algo a visitados")

def NodoMasCercano(Nodo, NodoEnd, vecinos):
    nodosOrdenados=[]
    nodosOrdenados=vecinos
    for index, i in enumerate(nodosOrdenados):
        if(index < len(nodosOrdenados)-1):
            # print("NODO CERCANO")
            if(Distancia2Puntos(nodosOrdenados[index], NodoEnd)<Distancia2Puntos(vecinos[int(index)+1], NodoEnd)):
                temp=nodosOrdenados[index]
                nodosOrdenados[index]=nodosOrdenados[index+1]
                nodosOrdenados[index+1]=temp
